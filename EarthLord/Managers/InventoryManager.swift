...
final class InventoryManager: ObservableObject {
    ...
    static func classifyDisassembleMaterial(from name: String, description: String? = nil, fallback: String) -> String {
        ...
        let electronic = ["电池", "显卡", "芯片", "电路", "电子", "模块", "充电", "充电宝", "信号", "天线",
                          ...
                          "手机", "电话", "平板", "电脑", "笔记本", "相机", "收音机", "对讲机", "计算器",
                          "导航", "GPS", "打印机", "扫描", "键盘", "鼠标", "耳机", "音响", "播放器",
                          "充电器", "电器", "仪器", "仪表", "探测器", "4G", "5G", "WiFi", "蓝牙"]
        ...
        // 文件/纸张类 → 废金属（回收利用价值低，归废料）
        let paper = ["文件", "单据", "表格", "档案", "记录", "清单", "账本", "合同", "物流单",
                     "发货单", "收据", "报告", "手册", "指南", "说明书", "证书", "票据", "票"]
        if paper.contains(where: { combined.contains($0) }) { return "scrap_metal" }

        // 布料/纤维/泡沫/塑料类 → 布料
        let cloth = ["布料", "棉布", "纤维", "织物", "丝绸", "麻布", "帆布", "皮革", "衣物",
                     "编织", "毯子", "袋子", "画册", "书本", "纸张", "本子",
                     "泡沫", "塑料", "橡胶", "尼龙", "硅胶", "海绵", "包装",
                     "滤芯", "过滤芯", "滤网", "面罩", "防护罩", "口罩"]
        ...
        // 食物/有机物类 → 燃料（有机物可燃）
        let food = ["汉堡", "炸鸡", "薯条", "披萨", "便当", "食物", "食品", "熟食", "罐头食品",
                    "饼干", "面包", "蛋糕", "糕点", "零食", "肉干", "肉罐", "午餐肉"]
        if food.contains(where: { combined.contains($0) }) { return "fuel" }
